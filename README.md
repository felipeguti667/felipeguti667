<!--
  ================================================================
  README.md — Perfil GitHub estilo J.A.R.V.I.S / Cyberpunk
  ================================================================
  COMO PERSONALIZAR (leia antes de publicar):
  1) Substitua todas as ocorrências de:
     - SEU_USUARIO      → felipeguti667
     - Seu Nome         → Felipe Gutierrez Domingues
     - seu@email.com    → felipeguti667@gmail.com
     - seu-usuario      → seu usuário do LinkedIn/Twitter/Instagram, etc.
     - links de projetos e imagens marcados com [link-do-projeto]
  2) O Snake Animation (contribution graph animado) exige configurar
     uma GitHub Action no seu repositório SEU_USUARIO/SEU_USUARIO.
     As instruções estão comentadas na seção correspondente abaixo.
  3) Crie um repositório especial chamado exatamente igual ao seu
     usuário (ex: github.com/SEU_USUARIO/SEU_USUARIO) e cole este
     conteúdo no README.md dele — é isso que faz aparecer no seu perfil.
  ================================================================
-->

<div align="center">

<!-- BANNER PRINCIPAL — efeito de inicialização de sistema -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f0c29,50:302b63,100:24243e&height=280&section=header&text=SYSTEM%20ONLINE&fontSize=60&fontColor=219ebc&animation=fadeIn&fontAlignY=38&desc=Initializing%20Profile%20of%20Seu%20Nome...&descAlignY=58&descSize=20&descColor=250902" width="100%"/>

<br/>

<!-- TYPING ANIMATION -->
<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=26&duration=3000&pause=800&color=219ebc&center=true&vCenter=true&multiline=true&repeat=true&width=650&height=100&lines=%3E+Booting+neural+interface...;Ol%C3%A1%2C+eu+sou+Seu+Nome+%F0%9F%A4%96;Full-Stack+Developer+%7C+AI+Enthusiast;Compilando+ideias+em+c%C3%B3digo+desde+20XX" alt="Typing SVG" />
</a>

<br/><br/>

<img src="https://img.shields.io/badge/STATUS-ONLINE-00F0FF?style=for-the-badge&labelColor=0d1117" />
<img src="https://img.shields.io/badge/MODO-DESENVOLVEDOR-8A2BE2?style=for-the-badge&labelColor=0d1117" />
<img src="https://img.shields.io/badge/LOCALIZAÇÃO-BRASIL-00FF9C?style=for-the-badge&labelColor=0d1117" />

</div>

<br/>

<!-- ============================================================ -->
<!-- SOBRE MIM -->
<!-- ============================================================ -->

## <img src="https://media.giphy.com/media/iY8CRBdQXODJSCERIr/giphy.gif" width="35"/> `> sobre_mim`

<img align="right" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif" width="320">

``` sobre_mim
usuario: "Felipe Gutierrez Domingues"
cargo: "Full-Stack Developer & AI Explorer"
localizacao: "Brasil"
foco_atual: "Sistemas inteligentes e interfaces do futuro"
linguagem_principal: "JavaScript / Python"
curiosidade: "Movido a Monster e ficção científica ☕🤖"
filosofia: "Construir hoje a tecnologia de amanhã"
```

- 🔭 Atualmente trabalhando em **[há procura]**
- 🌱 Aprendendo **[me especializando em back end atualmente]**
- 👯 Aberto a colaborar em **[criação de sites, e contribuição com diversas empresas]**
- 💬 Pergunte-me sobre **[Tecnologia e planos futuros]**
- ⚡ Fato curioso: **[sempre buscando me aperfeiçoar]**

<br clear="right"/>

---

<!-- ============================================================ -->
<!-- TECH STACK -->
<!-- ============================================================ -->

## <img src="https://media.giphy.com/media/WFZvB7VIXBgiz3oDXE/giphy.gif" width="35"/> `> tech_stack --list`

<div align="center">

### Linguagens
<img src="https://skillicons.dev/icons?i=js,ts,python,java,cpp,html,css&theme=dark" />

### Frontend
<img src="https://skillicons.dev/icons?i=react,vue,nextjs,tailwind,redux,vite&theme=dark" />

### Backend
<img src="https://skillicons.dev/icons?i=nodejs,express,django,flask,fastapi,graphql&theme=dark" />

### Banco de Dados
<img src="https://skillicons.dev/icons?i=mongodb,postgres,mysql,redis,supabase,firebase&theme=dark" />

### DevOps & Cloud
<img src="https://skillicons.dev/icons?i=docker,kubernetes,aws,gcp,azure,githubactions&theme=dark" />

### IA & Dados
<img src="https://skillicons.dev/icons?i=tensorflow,pytorch,opencv&theme=dark" />
<img src="https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white" />
<img src="https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white" />
<img src="https://img.shields.io/badge/Anthropic_Claude-D97757?style=for-the-badge&logo=anthropic&logoColor=white" />

</div>

---

<!-- ============================================================ -->
<!-- FERRAMENTAS -->
<!-- ============================================================ -->

## <img src="https://media.giphy.com/media/WFZvB7VIXBgiz3oDXE/giphy.gif" width="35"/> `> tools --active`

<div align="center">

<img src="https://skillicons.dev/icons?i=git,github,vscode,figma,postman,linux,vercel,npm&theme=dark" />

</div>

---

<!-- ============================================================ -->
<!-- PROJETOS EM DESTAQUE -->
<!-- ============================================================ -->

## <img src="https://media.giphy.com/media/du3J3cXyzhj75IOgvA/giphy.gif" width="35"/> `> projects --featured`

<div align="center">

<table>
<tr>
<td width="50%">

### 🛰️ [Doacao-animais](https://github.com/felipeguti667/doacao-animais.git)
> Site de doações e acolhimento de animais de situação de rua

`React + Vite` `JavaScript` `CSS`

[![Repo](https://img.shields.io/badge/-Ver_Repositório-0d1117?style=for-the-badge&logo=github&logoColor=00F0FF)](https://github.com/felipeguti667/doacao-animais.git)

</td>
<td width="50%">

### 🧠 [media-eniac](https://github.com/felipeguti667/media-eniac.git)
> programa para calcular a media das matérias regulares e de projeto da faculdade Eniac (sempre atualizado)

`Python`

[![Repo](https://img.shields.io/badge/-Ver_Repositório-0d1117?style=for-the-badge&logo=github&logoColor=00F0FF)](https://github.com/felipeguti667/media-eniac.git)

</td>
</tr>
<tr>
<td width="50%">

### 🚀 [Em progresso...](link-do-projeto-3)
> ...

`.` `.` `.`

[![Repo](https://img.shields.io/badge/-Ver_Repositório-0d1117?style=for-the-badge&logo=github&logoColor=00F0FF)](link-do-projeto-3)

</td>
<td width="50%">

### 🔐 [Em progresso...](link-do-projeto-4)
> ...

`.` `.` `.`

[![Repo](https://img.shields.io/badge/-Ver_Repositório-0d1117?style=for-the-badge&logo=github&logoColor=00F0FF)](link-do-projeto-4)

</td>
</tr>
</table>

</div>

---

<!-- ============================================================ -->
<!-- OBJETIVOS ATUAIS — ESTILO TERMINAL -->
<!-- ============================================================ -->

## <img src="https://media.giphy.com/media/LMt9638dO8dftAjtco/giphy.gif" width="35"/> `> current_goals.sh --run`

```bash
felipeguti667@github:~$ ./roadmap_2026.sh

[✔] Dominar arquitetura de microsserviços
[✔] Contribuir para projetos open-source
[▓▓▓▓▓▓▓░░░] 70%  Aprofundar em LLMs e Agentes de IA
[▓▓▓▓░░░░░░] 40%  Certificação em Cloud (AWS/GCP)
[▓▓░░░░░░░░] 20%  Publicar artigo técnico sobre automação

> status: em_progresso
> próxima_missão: "aumentar o maximo de portfolios possiveis"
> log: sistema aprendendo continuamente...
```

---

<!-- ============================================================ -->
<!-- GITHUB STATS -->
<!-- ============================================================ -->

## <img src="https://media.giphy.com/media/L1R1tvI9svkIWwpVYr/giphy.gif" width="35"/> `> analytics --render`

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=SEU_USUARIO&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=00F0FF&icon_color=8A2BE2&text_color=c9d1d9&ring_color=00F0FF" />
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=SEU_USUARIO&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=00F0FF&text_color=c9d1d9&langs_count=8" />

<br/>

<img src="https://streak-stats.demolab.com?user=SEU_USUARIO&theme=tokyonight&hide_border=true&background=0D1117&ring=00F0FF&fire=8A2BE2&currStreakLabel=00F0FF" />

<br/>

<img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=SEU_USUARIO&theme=tokyo-night&hide_border=true&bg_color=0d1117&color=00F0FF&line=8A2BE2&point=ffffff&area=true&area_color=8A2BE2" />

</div>

---

<!-- ============================================================ -->
<!-- GITHUB TROPHIES -->
<!-- ============================================================ -->

## <img src="https://media.giphy.com/media/QssGEmpkyEOhBCb7e1/giphy.gif" width="35"/> `> achievements --unlocked`

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=SEU_USUARIO&theme=algolia&no-frame=true&no-bg=true&margin-w=12&margin-h=12&row=1&column=7" />

</div>

---

<!-- ============================================================ -->
<!-- SNAKE CONTRIBUTION ANIMATION -->
<!-- ============================================================ -->

## <img src="https://media.giphy.com/media/kH1DBFystsGSXQqcpp/giphy.gif" width="35"/> `> contribution_matrix.exe`

<!--
  Para ativar esta animação:
  1) Crie um arquivo em: .github/workflows/snake.yml no repositório felipeguti667/felipeguti667
  2) Cole o conteúdo abaixo:

  name: Generate Snake
  on:
    schedule:
      - cron: "0 0 * * *"
    workflow_dispatch: {}
    push:
      branches: [ main ]
  jobs:
    build:
      runs-on: ubuntu-latest
      steps:
        - uses: Platane/snk@v3
          with:
            github_user_name: felipeguti667
            outputs: |
              dist/github-contribution-grid-snake.svg
              dist/github-contribution-grid-snake-dark.svg?palette=github-dark
        - uses: crazy-max/ghaction-github-pages@v4
          with:
            target_branch: output
            build_dir: dist
          env:
            GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}

  3) Após a Action rodar, a imagem abaixo passará a exibir a cobra
     "comendo" seus contribution graphs automaticamente.
-->

<div align="center">
  <img src="https://raw.githubusercontent.com/felipeguti667/felipeguti667/output/github-contribution-grid-snake-dark.svg" width="100%" />
</div>

---

<!-- ============================================================ -->
<!-- REDES SOCIAIS -->
<!-- ============================================================ -->

## <img src="https://media.giphy.com/media/xTiTnxpQ3ghPiB2Hp6/giphy.gif" width="35"/> `> connect --channels`

<div align="center">

<a href="https://www.linkedin.com/in/felipe-gutierrez-67345b3b1/" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-00F0FF?style=for-the-badge&logo=linkedin&logoColor=black" />
</a>
<a href="https://www.instagram.com/felipekgutierrezk/" target="_blank">
  <img src="https://img.shields.io/badge/Instagram-8A2BE2?style=for-the-badge&logo=instagram&logoColor=white" />
</a>
<a href="mailto:felipeguti667@gmail.com" target="_blank">
  <img src="https://img.shields.io/badge/Email-00FF9C?style=for-the-badge&logo=gmail&logoColor=black" />
</a>
<a href="doacao-animais-pi.vercel.app" target="_blank">
  <img src="https://img.shields.io/badge/Portfólio-0d1117?style=for-the-badge&logo=vercel&logoColor=00F0FF" />
</a>
<a href="https://discord.com/users/Gutitale" target="_blank">
  <img src="https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white" />
</a>

</div>

---

<!-- ============================================================ -->
<!-- RODAPÉ -->
<!-- ============================================================ -->

<div align="center">

### `> system.shutdown() ` <img src="https://media.giphy.com/media/WUlplcMpOCEmTGBtBW/giphy.gif" width="30">

<img src="https://komarev.com/ghpvc/?username=SEU_USUARIO&label=VISUALIZAÇÕES+DO+PERFIL&color=00F0FF&style=for-the-badge" />

*"A tecnologia mais avançada é indistinguível da magia — eu apenas escrevo o feitiço em código."*

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:24243e,50:302b63,100:0f0c29&height=150&section=footer" width="100%"/>

</div>
